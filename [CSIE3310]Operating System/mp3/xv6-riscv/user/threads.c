...
void schedule(void){
    ...
    {
        rr_cnt++;
        if (rr_cnt == time_quantum || current_thread->is_exited || current_thread->is_yield)
        {
            rr_cnt = 0;
            current_thread = current_thread->next;
        }
    ...
    if (is_thread_start == 0)
    {
        struct thread *head = current_thread, *cur = current_thread->next;
        while (cur != current_thread)
        {
            if (cur->remain_execution_time < head->remain_execution_time ||
                (cur->remain_execution_time == head->remain_execution_time && cur->ID < head->ID))
            {
                head = cur;
            }
            cur = cur->next;
    ...
    {
        struct thread *head = current_thread->next, *cur = current_thread->next->next;
        while (cur != current_thread)
        {
            if (cur->remain_execution_time < head->remain_execution_time ||
                (cur->remain_execution_time == head->remain_execution_time && cur->ID < head->ID))
